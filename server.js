...
// ── SECRET MODE USAGE ───────────────────────────────────────────────────────
// GET: ambil penggunaan secret hari ini
app.post("/api/secret/usage", (req, res) => {
    const { email } = req.body;
    if (!email || email === "guest@app.local") return res.json({ count: 0, limit: 2, plan: "free" });
    try {
        const today = new Date().toISOString().slice(0, 10);
        const user = db.prepare("SELECT plan FROM users WHERE email = ?").get(email);
        const plan = user ? (user.plan || "free") : "free";
        const row = db.prepare("SELECT count FROM usage WHERE email = ? AND date = ?").get(email, today);
        const count = row ? row.count : 0;
        const limit = plan === "unlimited" || plan === "pro" ? -1 : plan === "premium" ? 20 : 2;
        res.json({ count, limit, plan });
    } catch(e) { res.status(500).json({ error: e.message }); }
});

// POST: tambah 1 penggunaan secret hari ini
app.post("/api/secret/increment", (req, res) => {
    const { email } = req.body;
    if (!email || email === "guest@app.local") return res.json({ success: true, count: 1 });
    try {
        const today = new Date().toISOString().slice(0, 10);
        const user = db.prepare("SELECT plan FROM users WHERE email = ?").get(email);
        const plan = user ? (user.plan || "free") : "free";
        ...
        // Cek limit dulu di server — anti-cheat
        const row = db.prepare("SELECT count FROM usage WHERE email = ? AND date = ?").get(email, today);
        const count = row ? row.count : 0;
        if (limit !== -1 && count >= limit) {
            return res.status(403).json({ error: "limit_reached", count, limit, plan });
        }

        // Upsert count
        db.prepare("INSERT INTO usage (email, date, count) VALUES (?, ?, 1) ON CONFLICT(email, date) DO UPDATE SET count = count + 1").run(email, today);
        const newCount = (row ? row.count : 0) + 1;
        res.json({ success: true, count: newCount, limit, plan });
    } catch(e) { res.status(500).json({ error: e.message }); }
});
